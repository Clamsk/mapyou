...
# ==================== 城市代码映射表 ====================
# 百度地图城市代码（来自官方对照表 BaiduMap_cityCode_1102.txt）
CITY_CODES = {
    # 直辖市
    "北京": "131", "北京市": "131",
    "上海": "289", "上海市": "289",
    ...
    "图木舒克": "792", "图木舒克市": "792",
    # 港澳台
    "香港": "2912", "香港特别行政区": "2912",
    "澳门": "2911", "澳门特别行政区": "2911",
    "台北": "9002", "台北市": "9002",
    "高雄": "9019", "高雄市": "9019",
    "台中": "9017", "台中市": "9017",
    "台南": "9016", "台南市": "9016",
...
def bd09_to_mercator(bd_lng, bd_lat):
    """BD-09经纬度转百度墨卡托坐标"""
    ...
    x = cE[0] + cE[1] * abs(bd_lng)
    tmp = abs_lat / cE[9]
    y = cE[2] + cE[3] * tmp + cE[4] * tmp * tmp + cE[5] * tmp ** 3 + \
        cE[6] * tmp ** 4 + cE[7] * tmp ** 5 + cE[8] * tmp ** 6

    x = x * (1 if bd_lng >= 0 else -1)
    y = y * (1 if bd_lat >= 0 else -1)

    return x, y


def wgs84_to_bd09_mercator(wgs_lng, wgs_lat):
    """WGS-84经纬度转百度墨卡托坐标"""
    bd_lng, bd_lat = wgs84_to_bd09(wgs_lng, wgs_lat)
    return bd09_to_mercator(bd_lng, bd_lat)
...
class CrawlerWorker(QThread):
    ...
    def load_geojson(self, geojson_path):
        ...
        try:
            ...
            name_from_file = os.path.splitext(filename)[0]  # 去掉扩展名

            # 如果文件名是区/县名，补全市名
            if name_from_file in DISTRICT_TO_CITY:
                city = DISTRICT_TO_CITY[name_from_file]
                self.region_name = f"{city}市{name_from_file}"
            elif name_from_file:
                self.region_name = name_from_file
    ...
    def is_in_boundary(self, lng, lat):
        ...
        注意：传入的是BD-09坐标，边界文件通常是WGS-84坐标
        需要先将BD-09转换为WGS-84再比较
        """
...
class MainWindow(QMainWindow):
    ...
    def set_window_icon(self):
        ...
        icon_paths = [
            # 打包后的路径（与exe同目录）
            os.path.join(os.path.dirname(sys.executable), '图标.png'),
            os.path.join(os.path.dirname(sys.executable), 'icon.png'),
            # 开发时的路径
            os.path.join(os.path.dirname(os.path.abspath(__file__)), '图标.png'),
    ...
    def init_ui(self):
        ...
        self.region_input.setPlaceholderText("必须包含市名！例如: 福州市鼓楼区")
...
def main():
    ...
    splash = None
    try:
        icon_path = Path(__file__).resolve().parent / "图标.png"
        if icon_path.exists():
            pixmap = QPixmap(str(icon_path))
            if not pixmap.isNull():
                scaled = pixmap.scaled(480, 360, Qt.KeepAspectRatio, Qt.SmoothTransformation)
